                ...
                <div className="flex items-center gap-3 lg:gap-5">
                    {user ? (
                        <>
                            ...
                            {isAdmin && (
                                <Link to="/admin" className="hidden sm:flex flex-col items-center cursor-pointer group">
                                    ...
                                    <span className="text-xs text-gray-500 mt-1 font-medium group-hover:text-primary">Admin</span>
                                </Link>
                            )}
                            <button onClick={() => { logout(); navigate('/'); }} className="hidden sm:flex flex-col items-center cursor-pointer group" title="Log Out">
                                <svg className="w-6 h-6 text-gray-500 group-hover:text-danger transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
                                <span className="text-xs text-gray-500 mt-1 font-medium group-hover:text-danger">Logout</span>
                            </button>
                        </>
                    ) : (
                        <>
                            {/* Not logged in: Cart + Login/Register buttons */}
                            <Link to="/cart" className="flex flex-col items-center group relative">
                                <svg className="w-6 h-6 text-gray-500 group-hover:text-primary transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                                <span className="text-xs text-gray-500 mt-1 font-medium group-hover:text-primary hidden sm:block">My cart</span>
                                {cartCount > 0 && (
                            ...
                            <Link to="/login" className="hidden sm:inline-flex items-center px-5 py-2 bg-primary text-white text-sm font-bold rounded-lg hover:bg-blue-700 transition-colors shadow-sm">
                                Login
                            </Link>
                            <Link to="/register" className="hidden sm:inline-flex items-center px-5 py-2 bg-white text-primary text-sm font-bold rounded-lg border-2 border-primary hover:bg-blue-50 transition-colors">
                                Register
                            </Link>
                        </>
                    )}
